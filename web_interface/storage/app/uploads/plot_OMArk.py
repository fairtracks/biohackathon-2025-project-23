...
def plot_omark(output_path):
    ...
    if __name__=='__main__':
        ...
        if len(main_df)>0:
            ...
            def plot_omark(omark_folder, output_img):
                """
                Reads OMArk results from a folder and generates a plot image.
                """
                # Find all OMArk result folders
                folders = [os.path.join(omark_folder, x) for x in os.listdir(omark_folder) if os.path.isdir(os.path.join(omark_folder, x))]
                if not folders:
                    raise ValueError(f"No OMArk result folders found in {omark_folder}")
                # Use existing functions to create DataFrame and plot
                main_df, cont_df = create_df_from_results(folders)
                if len(main_df) > 0:
                    plot_omark_df(main_df, savefile=output_img)
                else:
                    raise ValueError('No data was found in this folder.')

            # ...existing code for parse_sum_file, create_df_from_results, integrate_external_data, read_mapping_file, plot_omark_df...

            if __name__ == "__main__":
                import sys
                if len(sys.argv) < 3:
                    print("Usage: python plot_OMArk.py <omark_folder> <output_img>")
                    sys.exit(1)
                plot_omark(sys.argv[1], sys.argv[2])